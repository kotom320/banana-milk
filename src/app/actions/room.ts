...
export async function createRoom(
  ...
  teamMethod: TeamMethod = 'balanced'
) {
  ...
  const { assignTeams } = await import('@/lib/team-balancer')
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const assignment = assignTeams(players as any, teamCount, teamMethod)

  const { data: room, error: rErr } = await supabase
    .from('rooms')
    .insert({ title, team_count: teamCount, scoring_rule: scoringRule, team_method: teamMethod })
    .select()
    .single()
